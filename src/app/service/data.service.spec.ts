...
describe('DataService', () => {
  ...
  describe('storeValue', () => {
    it('should add data to storedData', () => {
      ...
      expect(service.storedData()).toContain(data);
      ...
      const isIso8601 = !isNaN(
        Date.parse(service.storedData()[0].date.toISOString())
      );
  ...
  describe('clearValues', () => {
    it('should clear storedData', () => {
      service.storeValue({ date: new Date(), amount: 100 });
      service.clearValues();
      expect(service.storedData()).toEqual([]);
    });
  });
});
